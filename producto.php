...
<head>
  ...
  <style>
    ...
    .product-detail-section { background-color: var(--white); border-radius: 1.25rem; padding: 3rem; margin-top: 2rem; }
    ...
    .main-image-container { width: 100%; height: 400px; border-radius: 1rem; overflow: hidden; border: 1px solid var(--divider); cursor: pointer; position: relative; }
    .main-image { width: 100%; height: 100%; object-fit: cover; transition: transform 0.3s ease; }
    .main-image:hover { transform: scale(1.05); }
    .zoom-hint { position: absolute; bottom: 10px; right: 10px; background: rgba(0,0,0,0.6); color: white; padding: 5px 10px; border-radius: 20px; font-size: 0.8rem; pointer-events: none; }
    
    .thumbnails-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(80px, 1fr)); gap: 0.5rem; }
    .thumbnail { height: 80px; border-radius: 0.5rem; overflow: hidden; cursor: pointer; opacity: 0.7; transition: all 0.2s; border: 2px solid transparent; }
    .thumbnail.active, .thumbnail:hover { opacity: 1; border-color: var(--brown); }
    .thumbnail img { width: 100%; height: 100%; object-fit: cover; }

    /* Info */
    .product-info h1 { font-family: 'Lora', serif; font-size: 2.5rem; color: var(--black-blue); margin-bottom: 0.5rem; }
    .product-price { font-size: 1.5rem; color: var(--brown); font-weight: 700; margin-bottom: 1.5rem; }
    .product-meta { background: var(--off-white-light); padding: 1.5rem; border-radius: 1rem; margin-bottom: 2rem; }
    .meta-item { display: flex; justify-content: space-between; padding: 0.5rem 0; border-bottom: 1px solid var(--divider); }
    .meta-item:last-child { border-bottom: none; }
    .meta-label { font-weight: 600; color: var(--text-color); }
    .meta-value { font-weight: 500; color: var(--black-blue); }
    .product-description { margin-bottom: 2rem; line-height: 1.8; color: var(--text-color); }
    
    /* Botones de Acción - UX Mejorada */
    ...
    .btn-cart:hover { background: #2c3e50; transform: translateY(-2px); }

    /* Botón Primario: Comprar Ahora (Destacado) */
    .btn-buy {
        flex: 1;
    ...
    .btn-buy:hover { background: #6d4a2f; transform: translateY(-2px); box-shadow: 0 6px 20px rgba(139, 94, 60, 0.4); }
    
    .status-badge { display: inline-block; padding: 0.25rem 0.75rem; border-radius: 1rem; font-size: 0.875rem; font-weight: 600; margin-bottom: 1rem; }
    ...
    .modal-close:hover { background: rgba(139, 94, 60, 0.8); border-color: rgba(255, 255, 255, 0.4); transform: scale(1.1); }
    ...
    .modal-nav:hover { background: rgba(139, 94, 60, 0.8); border-color: rgba(255, 255, 255, 0.4); transform: translateY(-50%) scale(1.1); opacity: 1; }
    .modal-nav.prev { left: 2rem; }
    .modal-nav.next { right: 2rem; }

    .modal-image-container { padding: 1rem; display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; }
    .modal-image { max-width: 90vw; max-height: 85vh; object-fit: contain; border-radius: 8px; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.5); }
...
<body>
        ...
        <div class="logo-container">
         ...
         <div class="header-group">
          <div class="header-actions">
            <a href="#" id="themeToggle" class="btn-icon"><i class="fa-solid fa-moon"></i></a>
            ...
            <a href="/contacto.php" class="btn-text"><i class="fa-solid fa-phone"></i><span>Contáctanos</span></a>
          </div>
  ...
  <footer class="footer-container container-wide">
    <div class="container">
      <div class="footer-bottom">
        <div>&copy; <span id="current-year">2025</span> Rancho Las Trojes.</div>
      </div>
    </div>
  </footer>
  ...
  <script>
    // --- LÓGICA DE GALERÍA (IGUAL QUE ANTES) ---
    ...
    document.getElementById('hamburgerButton')?.addEventListener('click', function() {
        ...
        document.getElementById('navMenuOverlay').classList.toggle('active');
    });
  </script>
</body>
</html>